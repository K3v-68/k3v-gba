...
// Register the accepted loader request before it reaches the PSRAM controller.
// Besides making ownership explicit for the following cycle, this removes a
// same-cycle path from the GBA memory-mux idle decoder to the physical CRAM
// output registers. The loader holds address/data until ready is sampled.
...
// Pause new CPU/DMA work while the Pocket streams save data.  Existing flash
// program/erase work continues through gba_memorymux and must reach its exposed
// idle state before the first unload request is granted.  The timeout keeps an
// isolated diagnostic read from pausing gameplay indefinitely while remaining
// far longer than normal gaps between sequential bridge reads.
always @(posedge clk_sys) begin
    if (~pll_core_locked)
...
reg        save_load_seen;
reg        save_load_complete;
reg        save_load_failed;
reg        save_load_finalized;
reg        save_load_sequence_ok;
reg        save_fill_required;
reg [16:0] save_load_body_words;
wire [16:0] save_expected_body_words = det_flash_1m ? 17'd65536 : 17'd32768;
wire save_load_size_supported = det_flash_1m ?
    (save_load_body_words == 17'd65536) :
    ((save_load_body_words == 17'd256)   || // 512-byte EEPROM
     (save_load_body_words == 17'd4096)  || // 8 KiB EEPROM
     (save_load_body_words == 17'd16384) || // 32 KiB SRAM
     (save_load_body_words == 17'd32768));  // 64 KiB Flash
wire        save_finalize_safe;
reg  [4:0]  save_loader_idle_count;
wire        save_loader_settled = &save_loader_idle_count;

// dataslot_allcomplete crosses independently of the loader FIFO write pointer.
// Require 31 consecutive idle clk_sys cycles after allcomplete; any late FIFO
// visibility, target request, or physical PSRAM activity restarts the window.
always @(posedge clk_sys) begin
...
always @(posedge clk_sys) begin
    if (~pll_core_locked) begin
        ...
        // allcomplete can precede the final CDC FIFO drain. Finalize only
        // after every loader word has either been accepted or remains absent.
        if (!save_load_finalized && dataslot_allcomplete_s && save_finalize_safe) begin
            save_load_finalized <= 1'b1;
            if (save_load_seen) begin
                if (save_load_sequence_ok && save_load_size_supported) begin
                    if (save_load_body_words == save_expected_body_words)
                        save_load_complete <= 1'b1;
                    else
                        // Normalize a legitimate smaller EEPROM/SRAM import by
                        // filling its unused declared tail with erased bytes.
                        save_fill_required <= 1'b1;
                end else
                    // A partial/out-of-order file remains in reset and is
                    // never exposed to the Pocket unload writer.
            ...
            CLR_IDLE: begin
                if (save_load_finalized && !save_clear_done &&
                    ...
                    if (!save_load_seen) begin
                        clr_addr  <= 17'd0;
                        clr_state <= CLR_WR;
                    end else if (save_fill_required) begin
                        clr_addr  <= save_load_body_words;
                        clr_state <= CLR_WR;
                    end
                end
            end
...
always @(*) begin
    if (save_loader_grant) begin
        ...
        psram_addr       = save_loader_grant_addr;
        psram_data_in    = save_loader_grant_data;
        psram_write_high = 1;
        psram_write_low  = 1;
...
assign bus_client_idle = gba_save_bus_idle &&
                         (bus_state == BUS_IDLE) && !bus_out_ena;
    ...
    case (bus_state)
        ...
        BUS_SAVE_WAIT: begin
            if (bus_req_sent) begin
                bus_req_sent <= 0;
            end else if ((bus_is_write && !psram_busy) ||
                         (!bus_is_write && psram_read_avail)) begin
                if (!bus_is_write) begin
                    if (bus_byte_sel)
...
// Command 0x0090 is a one-shot boot notification.  Capture only the first
// complete event in clk_sys so duplicate notifications cannot re-anchor time.
reg [31:0] rtc_host_epoch;
reg [63:0] rtc_host_bcd;
reg        rtc_host_epoch_seen;
reg        rtc_host_bcd_seen;
reg        rtc_host_epoch_pulse;

always @(posedge clk_sys) begin
    rtc_host_epoch_pulse <= 1'b0;
    if (~pll_core_locked) begin
        rtc_host_epoch      <= 32'd0;
        rtc_host_bcd        <= 64'd0;
        rtc_host_epoch_seen <= 1'b0;
        rtc_host_bcd_seen   <= 1'b0;
    end else begin
        if (rtc_new_s && !rtc_host_epoch_seen) begin
            rtc_host_epoch       <= rtc_epoch_s;
            rtc_host_epoch_seen  <= 1'b1;
            rtc_host_epoch_pulse <= 1'b1;
        end
        if (rtc_bcd_new_s && !rtc_host_bcd_seen) begin
            rtc_host_bcd      <= rtc_bcd_s;
            rtc_host_bcd_seen <= 1'b1;
        end
    end
end
...
always @(posedge clk_sys) begin
    if (~pll_core_locked)
        rtc_save_loaded <= 0;
    else if (dataslot_allcomplete_s && rtc_data_captured &&
...
// Forward declaration; the menu-domain synchronizer is instantiated below.
wire force_rtc_s;
wire rtc_required_sys = quirk_gpio | force_rtc_s;
...
gba_top #(
    ...
    .savestate_number    ( 0 ),
    // RTC
    .RTC_timestampNew    ( rtc_host_epoch_pulse ),
    .RTC_timestampIn     ( rtc_host_epoch ),
    ...
    .save_sram           (),
    .save_flash          (),
    .save_bus_idle       ( gba_save_bus_idle ),
    .load_done           ( ss_load_done ),
    .savestate_busy      ( ss_busy ),
    .sleep_external      ( ss_loading | save_unload_pause ),
    // Input
    .KeyA                ( key_a ),
    .KeyB                ( key_b ),
